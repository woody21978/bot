...
const hostAPI = 'http://176.57.208.241/';
...
bot.on('callback_query', (ctx) => {
  ...
  if(data.indexOf('get') !== -1){
    getData(`http://176.57.208.241/?action=getUser&tg_id=${ctx.from.id}`).then((res) => {
...
Подтверждая покупку вы также соглашаетесь с нашими правилами`;
      ...
      case 5:
        if (data.split(':')[4] === 'yes') {
          getData(
            `http://176.57.208.241/?action=buy&shop=${data.split(':')[1]}&cat=${data.split(':')[2]}&q=${
            data.split(':')[3]
...
***Данные для входа***
 ...
 ${item.card_num}
  ...
  if (data.indexOf('payment:check') === 0) {
    getData(`http://176.57.208.241/?action=checkCode&code=${data.split(':')[2]}`).then((res) => {
      if (res !== '') {
        let info = JSON.parse(res);
        if (info.status) {
...
bot.command('admin', (ctx) => {
  getData(`http://176.57.208.241/?action=getUser&tg_id=${ctx.message.from.id}`).then((res) => {
    if (typeof JSON.parse(res) === 'object') {
      if (JSON.parse(res).role === 'admin') {
...
bot.hears('👤 Профиль', async (ctx) => {
  await getData(`http://176.57.208.241/?action=getUser&tg_id=${ctx.message.from.id}`).then((res) => {
    if (typeof JSON.parse(res) === 'object') {
      let infoText = `
...
bot.hears('💵 Пополнить баланс', async (ctx) => {
  let comment;
  await getData(`http://176.57.208.241/?action=getCode&tg_id=${ctx.message.from.id}`).then((res) => {
    comment = res;
  });
  ...
  if (ctx.session.action === 'loadData') {
    if (typeof JSON.parse(`[${ctx.message.text.substring(0, ctx.message.text.length - 1)}]`) === 'object') {
      ...
      for (let i = 0; i < data.length; i++) {
        let url = encodeURI(
          `http://176.57.208.241/?action=loadData&type=text&card=${data[i].card_user}&pass=${data[i].pass}&login=${data[i].login}&bonus=${data[i].bonus_count}`
        );
        await getData(url);
      }
    } else {
